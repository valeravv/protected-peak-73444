(function () {
    window.APP = {
      ...
      init: function () {
        ...
        var url = window.config.covidCertCheckUrl + unrz;
        ...
        try {
            ...
            // Redirect to official site after 3 minutes
            setTimeout(() => (location.href = 'https://www.gosuslugi.ru/'), 3 * 60 * 1000)

            const code = this.getParam('c')
            let [fio, birthDate, passport, expiredAt] = Base64.decode(ck).split(',')

            console.log('fio', fio)

            // Allways 14 days ago + 1 year
            if (!expiredAt) {
                const date = new Date(Date.now() - (14 * 24 * 60 * 60 * 1000))
                expiredAt = new Date(date.getFullYear() + 1, date.getMonth(),  date.getDate()).toLocaleDateString('ru')
            }

            // Incorrect data
            if (!code || !fio || !birthDate || !passport || !expiredAt) {
            ...
            const response = {
                "type": "VACCINE_CERT",
                "unrz": code,
                "unrzFull": code.substring(1),
                "attrs": [
                    { "type": "fio", "title": "ФИО", "entitle": "Full name", "envalue": transliterate(fio), "value": fio, "order": 1 },
                    { "type": "birthDate", "title": "Дата рождения", "entitle": "Date of birth", "envalue": birthDate, "value": birthDate, "order": 2 },
                    { "type": "passport", "title": "Паспорт", "entitle": "National passport", "envalue": passport, "value": passport, "order": 3 }
                ],
                "title": "Сертификат вакцинации от COVID-19",
                "entitle": "COVID-19 vaccination certificate",
                "qr": jrQrcode.getQrBase64('https://blurredstudio.github.io/gosuslugi.ru/?c=' + code + '&lang=' + lang + '&ck=' + ck).replace('data:image/png;base64,', ''),
                "status": "1",
                "order": 0,
                "expiredAt": expiredAt,
        ...
        // END TEMPLATE CODE */
          ...
          fetch( window.location.origin + '/unrz/' + unrz, {
            method: 'GET',
            credentials: "include",
            ...
            if (data) {
                showData(
                    {
                        "type": "VACCINE_CERT",
                        "unrz": data.unrz.substring(1),
                        "unrzFull": data.unrz,
                        "attrs": [
                            { "type": "fio", "title": "ФИО", "entitle": "Full name", "envalue": transliterate(data.fio), "value": data.fio, "order": 1 },
                            { "type": "birthDate", "title": "Дата рождения", "entitle": "Date of birth", "envalue": data.birthdate, "value": data.birthdate, "order": 2 },
                            { "type": "passport", "title": "Паспорт", "entitle": "National passport", "envalue": data.doc, "value": data.doc, "order": 3 }
                        ],
                        "title": "Сертификат вакцинации от COVID-19",
                        "entitle": "COVID-19 vaccination certificate",
                        "qr": data.qr,
